...
static uint8_t binary_val[] = {0b10101010, 0b01010101, 0b11001100, 0b00110011, 0b11110000, 0b00001111};
// number of elements in the array
uint16_t num = sizeof(binary_val) / sizeof(binary_val[0]);
// EEPROM start address 
uint16_t address = 0x0000; // EEPROM start address 
// for task 6
uint8_t is_button_pressed = 0; // Flag to track button press state
uint32_t delay_value = 1000; // Initial delay value in milliseconds (1 second)
...
int main(void)
{
  ...
  for (uint16_t i=0; i<num; i++) {
  	write_to_address(address + i, binary_val[i]);
  }
  ...
  while (1)
  {
	...
	{
		if (!is_button_pressed) // Execute this block only once when button is pressed
		{
			is_button_pressed = 1;
			...
			if (delay_value == 1000)
			{
				delay_value = 500; // Half-second delay
			}
			else
			{
				delay_value = 1000; // 1-second delay
			}
			
			// Update the TIM16 ARR value for the new delay
			htim16.Instance->ARR = (delay_value * 8) - 1; // Convert to TIM16 ticks
		}
		else
		{
			is_button_pressed = 0; 
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t read_value = read_from_address(address); 
	
	// Check the value read matches the expected value from the array
	if (read_value == binary_val[address]) {
		// Display the binary values on LEDs (example: LED0 to LED7)
		LL_GPIO_WriteOutputPort(LED0_GPIO_Port, read_value); // Placeholder for LED control
	} else {
		// Output 0b00000001 to LEDs to indicate SPI failure
		LL_GPIO_WriteOutputPort(LED0_GPIO_Port, 0b00000001); // Placeholder for LED control
	}
	
	// increment the address and wrap around if necessary
	address = (address + 1) % num;
