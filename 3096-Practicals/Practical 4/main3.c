...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
HAL_StatusTypeDef TC74A0_ReadTemp(uint8_t address, uint8_t *tempRead)
{
  HAL_StatusTypeDef ret;
  uint8_t data = 0x00;

  ret = HAL_I2C_Master_Receive(&hi2c2, (address<<1) | 0x01, &data, 1, 50);
  if (ret != HAL_OK)
  {
    return ret;
  }
...
int main(void)
{
  ...
  TC74A0_ReadTemp(temp_Address, &tempRead);
